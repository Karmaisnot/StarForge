...
function StudentDirectory({ route, onNav, branchId, user }) {
  ...
  // The API already limits teacher principals to students enrolled in their
  // assigned groups. Tenant-wide branch/teacher controls are therefore both
  // redundant and misleading in the teacher workspace.
  const canViewBranches = !teacherScoped && canUseCapability(user, 'org:read');
...
function EnrollmentView({ onNav, user }) {
  ...
  const teacherScoped = user?.accountKind === 'teacher';
  const scopeLabel = teacherScoped ? 'Your teaching scope' : 'Your permitted scope';
  ...
  return <div className="fw-analytics-grid">
    <div className="fw-summary-grid is-full">
      <div className="fw-summary-card"><span>Total students</span><strong>{displayCount(totalStudents)}</strong><small>{statsReady ? scopeLabel : 'Enrollment snapshot unavailable'}</small></div>
    ...
    <ChartCard eyebrow="Enrollment status" title="Current student mix" description={mix.length || totalStudents === 0 ? `${scopeLabel}; status counts reconcile to the recorded total.` : 'Status mix is withheld because its counts do not reconcile to a verified total.'}><DonutBreakdown data={mix} centerValue={displayCount(totalStudents)} centerLabel="students" /></ChartCard>
  ...
  return (
    <div className="fw-page">
      ...
      <div className="fw-layout-content">
        ...
        {section === 'enrollment' && <EnrollmentView onNav={onNav} user={user} />}
